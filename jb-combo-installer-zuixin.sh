...
CADDY_REQUIRED='${CADDY_REQUIRED:-0}'
WEB_ENABLED='${WEB_ENABLED:-0}'

MAIN_DOMAIN='${MAIN_DOMAIN:-}'
MAIN_DOMAIN2='${MAIN_DOMAIN2:-}'
XHTTP_DOMAIN='${XHTTP_DOMAIN:-}'
XHTTP_PATH='${XHTTP_PATH:-}'
XHTTP_UUID='${XHTTP_UUID:-}'
XHTTP_BACKEND_PORT='${XHTTP_BACKEND_PORT:-}'
...
HY2_PORT_MODE='${HY2_PORT_MODE:-}'
HY2_PORT='${HY2_PORT:-}'
HY2_FIRST_PORT='${HY2_FIRST_PORT:-}'
HY2_END_PORT='${HY2_END_PORT:-}'
HY2_HOP_INTERVAL='${HY2_HOP_INTERVAL:-25}'
HY2_MIN_HOP_INTERVAL='${HY2_MIN_HOP_INTERVAL:-}'
HY2_MAX_HOP_INTERVAL='${HY2_MAX_HOP_INTERVAL:-}'
HY2_CERT_SOURCE='${HY2_CERT_SOURCE:-}'
...
port_in_use_tcp() { ss -H -ltn "sport = :$1" 2>/dev/null | grep -q .; }
port_in_use_udp() { ss -H -lun "sport = :$1" 2>/dev/null | grep -q .; }
...
generate_hy2_outputs() {
  ...
  if [[ "$HY2_PORT_MODE" == "hop" ]]; then
    server_port_string="${HY2_PORT},${HY2_FIRST_PORT}-${HY2_END_PORT}"
  else
...
YAML
  ...
  python3 - <<PY > "$HY2_CLIENT_JSON"
import json
obj={"server":"${server_host}:${server_port_string}","auth":"${HY2_PASSWORD}","tls":{"sni":"${HY2_DOMAIN}","insecure":False},"quic":{"initStreamReceiveWindow":16777216,"maxStreamReceiveWindow":16777216,"initConnReceiveWindow":33554432,"maxConnReceiveWindow":33554432},"socks5":{"listen":"127.0.0.1:5678"}}
if "${HY2_PORT_MODE}" == "hop":
    udp={}
    if "${HY2_MIN_HOP_INTERVAL:-}" and "${HY2_MAX_HOP_INTERVAL:-}":
        udp["minHopInterval"]="${HY2_MIN_HOP_INTERVAL:-15}s"; udp["maxHopInterval"]="${HY2_MAX_HOP_INTERVAL:-25}s"
    else:
        udp["hopInterval"]="${HY2_HOP_INTERVAL:-25}s"
...
JSON
    ...
    reality)
      ...
      main_link="vless://${REALITY_UUID}@${REALITY_ADDRESS}:443?encryption=none&security=reality&sni=${REALITY_SNI}&fp=${FP}&pbk=${REALITY_PUBLIC_KEY}&sid=${REALITY_SHORT_ID}&type=tcp&flow=xtls-rprx-vision&spx=%2F#${name_enc}"
      cat > "$XRAY_CLIENT_JSON" <<JSON
{
  "outbounds": [
    {
...
collect_reality_hy2_inputs() {
  ...
  read -r -p "请输入 REALITY 连接地址（VPS IP 或域名）[默认 ${REALITY_ADDRESS}]: " ra
  REALITY_ADDRESS="${ra:-$REALITY_ADDRESS}"
  read -r -p "请输入 REALITY 目标 SNI [默认 nokia.com]: " rs
  REALITY_SNI="${rs:-nokia.com}"
  collect_fp
  REALITY_UUID="$(new_uuid)"
  local keys
  keys="$($XRAY_BIN x25519 2>/dev/null || true)"
  REALITY_PRIVATE_KEY="$(echo "$keys" | awk -F': ' '/Private key/ {print $2}' | head -n1)"
  REALITY_PUBLIC_KEY="$(echo "$keys" | awk -F': ' '/Public key/ {print $2}' | head -n1)"
...
full_uninstall() {
  ...
  apt-get purge -y caddy >/dev/null 2>&1 || true
  rm -f /usr/bin/caddy /usr/local/bin/jb /usr/local/lib/jb-combo-installer.sh
  rm -rf "$STATE_DIR"
  ok "彻底卸载完成。"
}
...
change_reality_target() {
  ...
  read -r -p "新的 REALITY 连接地址 [当前 ${REALITY_ADDRESS}]: " a
  REALITY_ADDRESS="${a:-$REALITY_ADDRESS}"
  read -r -p "新的 REALITY 目标 SNI [当前 ${REALITY_SNI}]: " s
