...
#   2) VLESS + REALITY + Vision + Hysteria2
#
# Design:
# - XHTTP stack uses Caddy for public TCP 80/443, certificates, and web camouflage.
...
# - Optional simple /swapfile setup runs before protocol service installation; default enabled, 0.7 GB; skipped when any swap is already configured.
# - Optional Hysteria2 Gecko obfuscation is available during HY2 setup; default disabled.
...
# 【已修正】将UI输出函数全部重定向到标准错误(>&2)，防止污染交互式变量捕获
...
save_state() {
  ...
  local vars=(
    ...
    HY2_OBFS_TYPE HY2_OBFS_PASSWORD HY2_GECKO_MIN_PACKET_SIZE HY2_GECKO_MAX_PACKET_SIZE
    SWAP_MANAGED SWAP_SIZE_GB SWAP_FILE_PATH
  )
  ...
  for name in "${vars[@]}"; do
    if [[ -v "$name" ]]; then
      ...
      case "$name" in
        ...
        HY2_OBFS_TYPE) val="none" ;;
        HY2_GECKO_MIN_PACKET_SIZE) val="512" ;;
        HY2_GECKO_MAX_PACKET_SIZE) val="1200" ;;
        SWAP_MANAGED) val="0" ;;
        SWAP_FILE_PATH) val="$SWAP_FILE" ;;
        *) val="" ;;
      esac
    fi
    printf '%s=%q\n' "$name" "$val" >> "$tmp"
...
resolve_self_source() {
  ...
  for candidate in "${JB_INSTALLER_SOURCE:-}" "${BASH_SOURCE[0]:-}" "$0"; do
    ...
    case "$candidate" in /dev/fd/*|/proc/*/fd/*) continue ;; esac
    [[ -f "$candidate" && -s "$candidate" && -r "$candidate" ]] || continue
    src="$candidate"
    break
  done
  [[ -n "$src" ]] || return 1
...
download_self_source() {
  local tmp
  tmp="${THIS_SCRIPT}.download.$$"
  rm -f "$tmp" 2>/dev/null || true

  if cmd_exists curl; then
    curl -fsSL "$SCRIPT_DOWNLOAD_URL" -o "$tmp"
  elif cmd_exists wget; then
    wget -qO "$tmp" "$SCRIPT_DOWNLOAD_URL"
  else
    rm -f "$tmp" 2>/dev/null || true
    return 1
  fi
  ...
  if ! bash -n "$tmp" >/dev/null 2>&1; then
    warn "下载到的控制面板脚本未通过 bash 语法检查，已拒绝覆盖本地脚本。"
    rm -f "$tmp" 2>/dev/null || true
    return 1
  fi

  install -m 755 "$tmp" "$THIS_SCRIPT"
  rm -f "$tmp" 2>/dev/null || true
  return 0
}
...
install_self_command() {
  ...
  if [[ "$rc" -eq 0 && -n "$src" ]]; then
    ...
    log "当前是一键远程运行模式，正在从 GitHub 刷新控制面板主脚本"
    if ! download_self_source; then
      if [[ -s "$THIS_SCRIPT" ]]; then
        warn "无法刷新控制面板脚本，暂时继续使用现有本地脚本：$THIS_SCRIPT"
        chmod 755 "$THIS_SCRIPT" 2>/dev/null || true
      else
        warn "无法保存控制面板脚本：系统没有 curl/wget 或下载失败。安装基础依赖后会再次尝试。"
        return 1
      fi
    fi
  fi
...
swap_size_to_mib() {
  ...
  awk -v s="$size" 'BEGIN {
    if (s < 0.1 || s > 5.0) exit 1;
    printf "%d", int(s * 1024 + 0.5);
  }'
}
...
swap_config_exists() {
  swap_active_exists || swap_fstab_exists
}

swap_config_summary() {
  local active fstab_entries
  active="$(awk 'NR > 1 { printf "%s(%s KiB) ", $1, $3 }' /proc/swaps 2>/dev/null | sed 's/[[:space:]]*$//')"
  fstab_entries=""
  if [[ -f /etc/fstab ]]; then
    fstab_entries="$(awk '
      /^[[:space:]]*#/ { next }
      NF >= 3 && $3 == "swap" { printf "%s ", $1 }
    ' /etc/fstab | sed 's/[[:space:]]*$//')"
  fi
  if [[ -n "$active" && -n "$fstab_entries" ]]; then
    printf 'active: %s; fstab: %s' "$active" "$fstab_entries"
  elif [[ -n "$active" ]]; then
    printf 'active: %s' "$active"
  elif [[ -n "$fstab_entries" ]]; then
    printf 'fstab: %s' "$fstab_entries"
  else
    printf 'none'
  fi
}

append_simple_swap_fstab_entry() {
  [[ -f /etc/fstab ]] || : > /etc/fstab
  backup_file /etc/fstab
  if ! awk -v p="$SWAP_FILE" '
      /^[[:space:]]*#/ { next }
      NF >= 3 && $1 == p && $3 == "swap" { found=1 }
      END { exit found ? 0 : 1 }
    ' /etc/fstab; then
...
setup_simple_swapfile() {
  local size_gb="$1" size_mib target_kib available_kib
  SWAP_MANAGED="0"
  SWAP_SIZE_GB="$size_gb"
  SWAP_FILE_PATH="$SWAP_FILE"

  if swap_config_exists; then
    ok "检测到系统已经配置了 Swap，跳过脚本创建 Swap：$(swap_config_summary)"
    return 0
  fi

  size_mib="$(swap_size_to_mib "$size_gb")" || {
    warn "Swap 大小必须在 0.1-5.0 GB 之间；已跳过 Swap，不影响节点安装。"
    return 0
  }
  target_kib=$((size_mib * 1024))

  if ! cmd_exists mkswap || ! cmd_exists swapon || ! cmd_exists dd; then
    log "安装 Swap 所需基础工具"
    install_base_packages || {
      warn "基础工具安装失败，已跳过 Swap，不影响节点安装。"
      return 0
    }
  fi

  if [[ -e "$SWAP_FILE" ]]; then
    warn "检测到 $SWAP_FILE 已存在。为避免误覆盖已有文件，已跳过创建 Swap。"
    warn "如确认该文件无用，可手动删除后重新运行脚本。"
    return 0
  fi

  available_kib="$(df -Pk "$(dirname "$SWAP_FILE")" 2>/dev/null | awk 'NR==2 {print $4}')"
  if [[ -n "$available_kib" ]] && (( available_kib < target_kib + 102400 )); then
    warn "磁盘剩余空间不足，无法创建 ${size_gb}GB Swap；请至少预留目标大小之外约 100MB 空间。已继续安装节点但不启用 Swap。"
    return 0
  fi

  log "正在创建 Swap 文件：$SWAP_FILE，大小约 ${size_gb}GB"
  if ! dd if=/dev/zero of="$SWAP_FILE" bs=1M count="$size_mib" status=none; then
    warn "写入 Swap 文件失败，已跳过 Swap，不影响节点安装。"
    rm -f "$SWAP_FILE" 2>/dev/null || true
    return 0
  fi
  chmod 600 "$SWAP_FILE" 2>/dev/null || true

  if ! mkswap "$SWAP_FILE" >/tmp/jb-combo-mkswap.log 2>&1; then
    warn "mkswap 失败，已跳过 Swap，不影响节点安装。"
    cat /tmp/jb-combo-mkswap.log 2>/dev/null || true
    rm -f "$SWAP_FILE" 2>/dev/null || true
    return 0
  fi

  if ! swapon "$SWAP_FILE" >/tmp/jb-combo-swapon.log 2>&1; then
    warn "swapon 失败，可能是容器/VPS 内核或文件系统限制；已删除脚本创建的 Swap 文件并继续安装节点。"
    cat /tmp/jb-combo-swapon.log 2>/dev/null || true
    rm -f "$SWAP_FILE" 2>/dev/null || true
    return 0
  fi
  ...
  append_simple_swap_fstab_entry
  SWAP_MANAGED="1"
  ok "Swap 已创建并挂载：$SWAP_FILE，大小约 ${size_gb}GB。"
}

remove_managed_swap() {
  # 按用户要求：脚本的任何卸载和删除功能都不触碰已经配置好的 Swap。
  return 0
}
...
prompt_swap_setup() {
  ...
  echo "  未检测到已有 Swap。默认启用 Swap，默认大小 0.7GB。"
  echo "  可输入范围：0.1-5.0，单位 GB。"
  read -r -p "是否创建 /swapfile 并启用 Swap？[Y/n]: " ans
  if [[ "${ans:-Y}" =~ ^[Nn]$ ]]; then
    SWAP_MANAGED="0"
    SWAP_SIZE_GB=""
    SWAP_FILE_PATH="$SWAP_FILE"
  ...
  local sg mib
  while true; do
    read -r -p "请输入 Swap 大小，单位 GB [默认 0.7]: " sg
    sg="${sg:-0.7}"
    if mib="$(swap_size_to_mib "$sg")"; then
      SWAP_SIZE_GB="$sg"
      break
    fi
    warn "Swap 大小格式不正确，必须是 0.1-5.0 之间的数字，例如 0.7、1、2.5。"
  done
  setup_simple_swapfile "$SWAP_SIZE_GB"
}
...
for raw_line in out.splitlines():
    ...
    key = re.sub(r"[^a-z0-9]+", "", raw_key.strip().lower())
    value = raw_value.strip()
    if not value:
        continue

    # Historical Xray output:      Private key: ... / Public key: ...
    # Newer Xray output variants: PrivateKey: ... / Password: ...
    # Current variant seen in the field: Password (PublicKey): ...
    if key == "privatekey" or key in {"private", "privkey"}:
        private = value
    elif key == "publickey" or key in {"public", "pubkey", "password", "passwordpublickey"}:
        public = value
    elif "publickey" in key and "hash" not in key:
        public = value
...
if not private:
    ...
    if m:
        private = m.group(1)
if not public:
    m = re.search(r"(?im)^\s*(?:Public\s*Key|Password\s*(?:\(\s*PublicKey\s*\))?)\s*:\s*(\S+)\s*$", out)
    if m:
        public = m.group(1)

# Guard against accidentally returning the Hash32 line as public key.
if public and re.search(r"(?im)^\s*Hash32\s*:\s*" + re.escape(public) + r"\s*$", out):
    public = ""
...
ensure_reality_keys_present() {
  if [[ -z "${REALITY_PRIVATE_KEY:-}" || -z "${REALITY_PUBLIC_KEY:-}" ]]; then
    warn "检测到 REALITY X25519 密钥缺失，正在重新生成。"
    generate_reality_x25519_keys
  fi
  ...
  if [[ -z "${REALITY_PRIVATE_KEY:-}" || -z "${REALITY_PUBLIC_KEY:-}" || -z "${REALITY_SHORT_ID:-}" ]]; then
    err "REALITY 密钥仍为空，已停止写入 Xray 配置，避免覆盖成坏配置。"
    exit 1
  fi
}
...
collect_fp() {
  echo "uTLS/客户端指纹：1) chrome  2) firefox" >&2
  read -r -p "请选择 [默认 1]: " c
...
collect_hy2_port() {
  ensure_port_tools
  echo
  echo "Hysteria2 端口模式：" >&2
  echo "  1) 端口跳跃 Port Hopping（默认，推荐）" >&2
  echo "  2) 单端口" >&2
  ...
  if [[ "${pm:-1}" == "2" ]]; then
    ...
    echo "端口跳跃间隔：1) 固定 25s（默认）  2) 随机 15-25s" >&2
...
collect_hy2_obfs() {
  echo >&2
  warn "Hysteria2 默认伪装为 HTTP/3；一般不建议开启混淆。"
  warn "只有在 HTTP/3/QUIC 流量遭到明显阻断、但 UDP 并未完全被封锁时，才建议尝试开启 Gecko 混淆。"
  warn "开启后服务端将不再兼容标准 QUIC/HTTP3 探测，客户端也必须配置完全一致的混淆密码。"
  echo "Hysteria2 混淆设置：" >&2
  echo "  1) 不启用混淆（默认，推荐）" >&2
  echo "  2) 启用 Gecko 混淆（实验性；使用默认分片 512-1200 字节）" >&2
  read -r -p "请选择 [默认 1]: " oc
  case "${oc:-1}" in
    2)
      HY2_OBFS_TYPE="gecko"
      read -r -p "Gecko 混淆密码 [回车自动生成]: " op
      HY2_OBFS_PASSWORD="${op:-$(generate_password)}"
...
write_reality_xray_config() {
  ...
  local tmp
  tmp="$(mktemp "${XRAY_CONFIG}.tmp.XXXXXX")"
...
JSON
  ...
  "$XRAY_BIN" run -test -config "$tmp" -format json >/tmp/xray-test.log 2>&1 || {
    err "Xray REALITY 配置测试失败"
...
hy2_obfs_yaml_block() {
  case "${HY2_OBFS_TYPE:-none}" in
    gecko)
      local passq
      passq="$(yaml_quote "${HY2_OBFS_PASSWORD:-}")"
      cat <<YAML
...
obfs:
  ...
  gecko:
    password: ${passq}
    minPacketSize: ${HY2_GECKO_MIN_PACKET_SIZE:-512}
    maxPacketSize: ${HY2_GECKO_MAX_PACKET_SIZE:-1200}
YAML
      ;;
    salamander)
      local passq
...
write_hy2_config_tls() {
  ...
  local certq keyq pwdq listen obfs_block
...
generate_hy2_outputs() {
  mkdir -p "$OUT_DIR"
  local server_host server_port_string auth_enc sni_enc link insecure="0" obfs_query="" obfs_pass_enc=""
  ...
  if [[ "${HY2_OBFS_TYPE:-none}" != "none" && -n "${HY2_OBFS_PASSWORD:-}" ]]; then
    obfs_pass_enc="$(urlencode "$HY2_OBFS_PASSWORD")"
    obfs_query="&obfs=$(urlencode "$HY2_OBFS_TYPE")&obfs-password=${obfs_pass_enc}"
  fi
  if [[ "$HY2_PORT_MODE" == "hop" ]]; then
    if [[ -n "${HY2_MIN_HOP_INTERVAL:-}" && -n "${HY2_MAX_HOP_INTERVAL:-}" ]]; then
      link="hysteria2://${auth_enc}@${server_host}:${HY2_PORT}?security=tls&mport=${HY2_FIRST_PORT}-${HY2_END_PORT}&mportHopInt=${HY2_MIN_HOP_INTERVAL}-${HY2_MAX_HOP_INTERVAL}${obfs_query}&insecure=${insecure}&sni=${sni_enc}#$(urlencode "${HY2_DOMAIN} HY2")"
    else
      link="hysteria2://${auth_enc}@${server_host}:${HY2_PORT}?security=tls&mport=${HY2_FIRST_PORT}-${HY2_END_PORT}&mportHopInt=${HY2_HOP_INTERVAL:-25}${obfs_query}&insecure=${insecure}&sni=${sni_enc}#$(urlencode "${HY2_DOMAIN} HY2")"
    fi
  else
    link="hysteria2://${auth_enc}@${server_host}:${HY2_PORT}?security=tls${obfs_query}&insecure=${insecure}&sni=${sni_enc}#$(urlencode "${HY2_DOMAIN} HY2")"
  fi
...
tls:
  ...
  insecure: false
YAML
  if [[ "${HY2_OBFS_TYPE:-none}" == "gecko" && -n "${HY2_OBFS_PASSWORD:-}" ]]; then
    cat >> "$HY2_CLIENT_YAML" <<YAML
obfs:
  type: gecko
  gecko:
    password: '$(printf "%s" "$HY2_OBFS_PASSWORD" | sed "s/'/''/g")'
    minPacketSize: ${HY2_GECKO_MIN_PACKET_SIZE:-512}
    maxPacketSize: ${HY2_GECKO_MAX_PACKET_SIZE:-1200}
YAML
  elif [[ "${HY2_OBFS_TYPE:-none}" == "salamander" && -n "${HY2_OBFS_PASSWORD:-}" ]]; then
    cat >> "$HY2_CLIENT_YAML" <<YAML
obfs:
  type: salamander
  salamander:
...
YAML
  ...
  HY2_JSON_OBFS_TYPE="${HY2_OBFS_TYPE:-none}" \
  HY2_JSON_OBFS_PASSWORD="${HY2_OBFS_PASSWORD:-}" \
  HY2_JSON_GECKO_MIN_PACKET_SIZE="${HY2_GECKO_MIN_PACKET_SIZE:-512}" \
  HY2_JSON_GECKO_MAX_PACKET_SIZE="${HY2_GECKO_MAX_PACKET_SIZE:-1200}" \
  python3 - <<'PY'
import json, os
obj = {
...
obfs_type = os.environ.get("HY2_JSON_OBFS_TYPE", "none")
obfs_password = os.environ.get("HY2_JSON_OBFS_PASSWORD", "")
if obfs_type == "gecko" and obfs_password:
    obj["obfs"] = {
        "type": "gecko",
        "gecko": {
            "password": obfs_password,
            "minPacketSize": int(os.environ.get("HY2_JSON_GECKO_MIN_PACKET_SIZE", "512")),
            "maxPacketSize": int(os.environ.get("HY2_JSON_GECKO_MAX_PACKET_SIZE", "1200")),
        },
    }
# 【已修正】将原本存在笔误的 obj_type 改正为声明的 obfs_type 变量
elif obfs_type == "salamander" and obfs_password:
    obj["obfs"] = {"type": "salamander", "salamander": {"password": obfs_password}}
...
Hysteria2 混淆: ${HY2_OBFS_TYPE:-none}
Hysteria2 Gecko 分片: ${HY2_GECKO_MIN_PACKET_SIZE:-512}-${HY2_GECKO_MAX_PACKET_SIZE:-1200}
Swap 创建: ${SWAP_MANAGED:-0}
Swap 文件: ${SWAP_FILE_PATH:-$SWAP_FILE}
Swap 大小GB: ${SWAP_SIZE_GB:-}
...
install_selected_stack() {
  case "$STACK_MODE" in
    ...
    *) err "未知或已移除的 STACK_MODE: $STACK_MODE"; exit 1 ;;
  esac
  print_result
}
...
choose_stack_initial() {
  ...
  echo "  2) VLESS + REALITY + Vision + Hysteria2"
  while true; do
    read -r -p "请输入选项 [1-2]: " ch
    case "$ch" in
      1) collect_xhttp_hy2_inputs; break ;;
      2) install_base_packages; install_xray_official; collect_reality_hy2_inputs; break ;;
      *) warn "请输入 1-2。" ;;
    esac
  done
}
...
regen_main_uuid_or_credential() {
  ...
  case "$MAIN_MODE" in
    ...
    *) warn "当前主节点模式已移除或不支持重新生成凭据：${MAIN_MODE:-unknown}"; return 0 ;;
  esac
...
change_fp() {
  load_state || return 1
  case "$MAIN_MODE" in xhttp|reality) collect_fp ;; *) warn "当前主节点模式无 uTLS 指纹设置或已不支持。"; return 0 ;; esac
  save_state; generate_main_outputs
  ok "客户端指纹参数已更新。"
}
...
restart_all() {
  ...
  case "${MAIN_MODE}" in
    ...
    *) warn "当前主节点模式已移除或不支持自动重启：${MAIN_MODE:-unknown}" ;;
  esac
...
menu() {
  ...
  if [[ "${STACK_MODE:-}" == "naive_hy2" || "${MAIN_MODE:-}" == "naive" ]]; then
    warn "检测到旧版 Naive + HY2 状态；新版已移除该组合。请切换到 XHTTP/REALITY，或执行彻底卸载。"
  fi
  while true; do
    echo
    echo "================ jb 组合节点控制面板 ================"
    ...
    echo "10) 卸载当前组合并安装 REALITY + Vision + HY2"
    echo "11) 彻底卸载"
    echo "0) 退出"
    read -r -p "请选择: " ch
    case "$ch" in
      ...
      10) switch_to_stack reality_hy2 ;;
      11) full_uninstall; exit 0 ;;
      0) exit 0 ;;
      *) warn "无效选项。" ;;
    esac
  done
...
print_result() {
  ...
  echo "详情文件：$INFO_FILE"
  echo
  echo "Hysteria2 混淆：${HY2_OBFS_TYPE:-none}"
  echo "Swap 创建：${SWAP_MANAGED:-0}  ${SWAP_FILE_PATH:-$SWAP_FILE} ${SWAP_SIZE_GB:-}GB"
  echo
  warn "注意："
  if [[ "${CADDY_REQUIRED:-0}" == "1" ]]; then
  ...
  if [[ "${HY2_OBFS_TYPE:-none}" != "none" ]]; then
    echo "5) 已启用 HY2 ${HY2_OBFS_TYPE} 混淆；客户端必须使用相同混淆类型和密码。若分享链接客户端不识别 gecko，请使用生成的 HY2 客户端 YAML/JSON。"
    echo "6) 已为相关 systemd 服务写入异常退出自动重启策略：Restart=on-failure，RestartSec=5s。"
  else
