...
# NaiveProxy needs a Caddy binary with the forwardproxy naive fork.
# Building it locally with Go/xcaddy can consume a lot of disk and RAM on tiny VPSes,
# so v7 tries Caddy's custom-download service first and only falls back to local build
# after resource checks.
NAIVE_FORWARDPROXY_PACKAGE="github.com/caddyserver/forwardproxy=github.com/klzgrad/forwardproxy@naive"
CADDY_DOWNLOAD_API="https://caddyserver.com/api/download"
NAIVE_BUILD_MIN_DISK_MB="${NAIVE_BUILD_MIN_DISK_MB:-1800}"
NAIVE_BUILD_MIN_MEM_MB="${NAIVE_BUILD_MIN_MEM_MB:-768}"
...
cleanup_naive_build_artifacts() {
  rm -rf /tmp/caddy-naive /tmp/caddy-naive.* /tmp/jb-xcaddy-* 2>/dev/null || true
  rm -rf /root/.cache/go-build 2>/dev/null || true
  apt-get clean >/dev/null 2>&1 || true
}

check_naive_local_build_resources() {
  local root_free tmp_free mem_avail
  root_free="$(disk_free_mb /)"; tmp_free="$(disk_free_mb /tmp)"; mem_avail="$(mem_available_mb)"
  root_free="${root_free:-0}"; tmp_free="${tmp_free:-0}"; mem_avail="${mem_avail:-0}"

  log "本地编译资源检查：/ 可用 ${root_free}MB，/tmp 可用 ${tmp_free}MB，内存+Swap 可用 ${mem_avail}MB"

  if (( root_free < NAIVE_BUILD_MIN_DISK_MB || tmp_free < NAIVE_BUILD_MIN_DISK_MB )); then
    err "磁盘空间不足，已拒绝本地编译 Naive Caddy，避免把 SSH/系统拖死。"
    err "至少建议预留 ${NAIVE_BUILD_MIN_DISK_MB}MB 可用空间；你这种 5GB 小盘应使用 v7 的远程预编译下载模式，或换 XHTTP/REALITY。"
    err "可先清理：apt-get clean; rm -rf /root/.cache/go-build /root/go /tmp/jb-xcaddy-* /tmp/caddy-naive*"
    return 1
  fi

  if (( mem_avail < NAIVE_BUILD_MIN_MEM_MB )); then
    err "内存/Swap 不足，已拒绝本地编译 Naive Caddy。Go 编译可能触发 OOM，导致 SSH 断开。"
    err "至少建议内存+Swap 可用 ${NAIVE_BUILD_MIN_MEM_MB}MB。"
    return 1
  fi
}

detect_caddy_download_arch() {
  case "$(uname -m)" in
...
caddy_binary_has_naive_forwardproxy() {
  local bin="${1:-$CADDY_BIN}"
  [[ -x "$bin" ]] || return 1
  "$bin" list-modules 2>/dev/null | grep -Eq '(^|[[:space:]])http\.handlers\.forward_proxy($|[[:space:]])|forward_proxy'
}

install_naive_caddy_from_download_api() {
  local arch pkg_enc url tmp
  arch="$(detect_caddy_download_arch)" || {
    warn "当前架构 $(uname -m) 暂不使用 Caddy 下载页预编译模式。"
    return 1
  }
  pkg_enc="$(urlencode "$NAIVE_FORWARDPROXY_PACKAGE")"
  url="${CADDY_DOWNLOAD_API}?os=linux&arch=${arch}&p=${pkg_enc}&idempotency=$(date +%s)-$$"
  tmp="$(mktemp /tmp/caddy-naive.XXXXXX)"

  log "从 Caddy 下载页获取带 Naive forwardproxy 的预编译 Caddy，避免在小 VPS 本地 Go 编译"
  if ! curl -fL --retry 3 --retry-delay 2 --connect-timeout 20 --max-time 900 "$url" -o "$tmp"; then
    rm -f "$tmp"
    warn "预编译 Caddy 下载失败，将尝试本地编译兜底。"
    return 1
  fi

  chmod 755 "$tmp"
  if ! "$tmp" version >/tmp/caddy-naive-version.log 2>&1; then
    warn "下载到的文件不是可执行 Caddy。响应摘要："
  ...
  if ! caddy_binary_has_naive_forwardproxy "$tmp"; then
    warn "下载的 Caddy 不包含 forward_proxy 模块，不能用于 Naive。模块列表如下："
    "$tmp" list-modules 2>/dev/null | sed -n '1,120p' || true
    rm -f "$tmp"
    return 1
  fi

  systemctl stop caddy >/dev/null 2>&1 || true
  install -m 755 "$tmp" "$CADDY_BIN"
  rm -f "$tmp"
  ok "Naive Caddy 预编译安装完成：$($CADDY_BIN version 2>/dev/null || echo unknown)"
  return 0
}

install_naive_caddy_local_build() {
  local had_go=0 build_dir gobin
  cmd_exists go && had_go=1
  cleanup_naive_build_artifacts
  check_naive_local_build_resources || return 1
  install_go_latest

  build_dir="$(mktemp -d /tmp/jb-xcaddy-XXXXXX)"
  export GOCACHE="${build_dir}/gocache"
  export GOPATH="${build_dir}/gopath"
  export GOMODCACHE="${build_dir}/gomodcache"
  gobin="${GOPATH}/bin"
  mkdir -p "$GOCACHE" "$GOPATH" "$GOMODCACHE" "$gobin"

  log "用 xcaddy 本地构建带 Naive forwardproxy 插件的 Caddy（资源较紧时可能较慢）"
  /usr/local/go/bin/go install github.com/caddyserver/xcaddy/cmd/xcaddy@latest
  "${gobin}/xcaddy" build --output /tmp/caddy-naive --with "$NAIVE_FORWARDPROXY_PACKAGE"

  if [[ ! -x /tmp/caddy-naive ]]; then
    rm -rf "$build_dir"
    err "Naive Caddy 本地构建失败。"
    exit 1
  fi
  if ! caddy_binary_has_naive_forwardproxy /tmp/caddy-naive; then
    rm -rf "$build_dir"
    err "本地构建出的 Caddy 不包含 forward_proxy 模块。"
    exit 1
  fi

  systemctl stop caddy >/dev/null 2>&1 || true
  install -m 755 /tmp/caddy-naive "$CADDY_BIN"
  rm -rf "$build_dir" /tmp/caddy-naive

  # 5GB 小盘上 Go 工具链和缓存很宝贵；如果 Go 是本脚本临时安装的，用完就移除。
  if (( had_go == 0 )); then
    rm -rf /usr/local/go
    rm -f /usr/local/bin/go /usr/local/bin/gofmt
  fi
  cleanup_naive_build_artifacts
  ok "Naive Caddy 本地构建完成：$($CADDY_BIN version 2>/dev/null || echo unknown)"
}
...
install_naive_caddy() {
  ...
  if install_naive_caddy_from_download_api; then
    cleanup_naive_build_artifacts
    return 0
  fi

  warn "预编译模式不可用，准备进入本地编译兜底。"
  install_naive_caddy_local_build || {
    err "Naive Caddy 安装失败。"
    err "小硬盘/小内存 VPS 建议：优先使用 XHTTP + HY2 或 REALITY + HY2；Naive 需要带第三方插件的 Caddy。"
    exit 1
  }
}
